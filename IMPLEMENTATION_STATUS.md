...
- **0.9.56（外部交接包：v2 回复格式偏差修复，基线 0.9.55 / `e2d9c70`）**：
  整合 `atlas-v0955-response-fix.patch`（五个文件；应用前核对五个 blob 哈希与补丁 index 行
  逐一相同，`git apply --check` 干净通过）。修两个已复现的格式偏差——已闭合的
  `<think>`/`<thinking>` 前缀与单层 ```json 围栏可剥离、完整对象后精确多余的 `"}`
  可修复（未闭合思考 / 截断 / 多对象 / 围栏后带正文 / 两种偏差叠加一律拒绝，绝不猜）；
  并让根路径 `$` 进得了安全诊断（JSON 解析失败时的唯一线索不再被白名单吞掉）。
  另跑 21 条对抗用例独立验证该启发式没有放宽。门禁：typecheck 0 errors；全量 632/632
  （0.9.55 的 628 → +4）；pack 通过。
  **仍未解决（不得描述为完整修复）**：① 证据引用不符——需要真实请求 / 失败回执里的
  msg:u 与 msg:a 原文才能定位（已排除「空来源」与「提示词/校验字节不一致」两种假设；
  现最强假设是模型把 `$7 前文剧情` 的句子挂到了 msg:a，而前文并不在可引用来源里）；
  ②「酒馆不出字」——世界模型 502 只证明世界未提交，需独立验证。
- **0.9.57（《Atlas 三表迁移、推进协议替换和地图接线执行计划》全量落地，分支 `codex/submap-v2`）**：
  按执行计划 A01–E08 逐编号实施完毕，执行记录与全部取舍见 `docs/三表迁移-实施决定.md`
  （D-01 … D-33）。本版要点：
  1. **三表成为实体现值的唯一权威**：`chatMetadata.atlas.tables`（地点 / 人物 / 物品三张表，
     schema 校验 + 上限 + 分支快照），旧 `world` 降级为供既有引擎读取的兼容镜像；
  2. **新推进协议 `table-delta-v1` 成为新装默认**：模型只输出 `<atlasEdit>` 行增量块，
     不再要求它写完整世界；协议由设置严格决定（不符即 `PROTOCOL_MISMATCH` 409，不猜、不偷偷换管线）；
  3. **地图 / 附近 / 子图同一数据口径**：`/state` 的 `tableMap` 通道 + UI 的地点、物品图钉、
     人物面板与附近页全部改读三表（含子图层与"建筑内 · 位置未知"名单）；
  4. **回退与写入口径一致**：`/turns/rollback` 用回合前三表快照还原（无快照则按回退后的世界重建）；
     拖动纠偏 / 地理提炼 / 开场识别都在同一事务里把三表补齐；
  5. **后台自主行动**（`src/atlas-background.ts`）：有目标、可到达、时间够走的 NPC 逐轮朝目标移动，
     上限 20 人/回合，太远只更新行动不瞬移；
  6. **世界书上下文**（E08）：每轮只注入位置链 + 身边人物的想法 / 行动倾向 + 地面物品，条数有界、分支隔离。
  门禁：`typecheck` 0 错；全量 **740/740（0 skip）**；`build` / `pack` 通过。
  旧协议 v1 / v2 完整保留（推进页可切）；`lib/` 一行未改。
  **真实 SillyTavern 实机验收仍未执行**——按既有发布口径先发版，实机问题随后回修。
