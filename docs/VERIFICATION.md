...
> 本文档记录 Atlas 在真实 SillyTavern 环境中需要验证的全部行为。当前自动化测试 488/488 通过——但**自动化测试不能证明真实酒馆命中、触摸、模型输出、刷新与旧档升级**。本清单是 R13 实机验收的最低门槛；任何一项失败都需在对应 Rxx 阶段补回。
...
- [ ] 启动时自动调 `core.reconcilePending()` 清 orphan pending（无 orphan 时报告 0/0）
