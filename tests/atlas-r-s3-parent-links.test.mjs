...
// ---------------------------------------------------------------------------
// 待办（S4 未完成的两项，需单独排查后再补）：
//
// 1) 同父孩子上限的**跨轮累计**验证：单响应最多 12 个新地点（MAX.newLocations），
//    因此 40 上限只能在多轮后触及。构造「已存 40 个子点」的夹具时 v2 解析器拒绝
//    （需打印 parsed.errors 定位是 points 上限还是别的约束），本条暂缺。
//    代码侧的判定已实现（childCount > V2_SUBMAP_SIBLINGS_MAX → 整轮拒绝），
//    但**尚无用例证明第 41 个被拒**，不得据此宣称已验收。
// 2) 同键重试返回 duplicate：applyAtlasV2Turn 是纯函数层，幂等去重由上层
//    commitAtlasTurn / 服务端 turn 映射承担；本文件误在纯函数层断言，需改到
//    服务层用例（atlas-server-plugin.test.mjs 的 0.9.31/T01 幂等夹具）验证。
// ---------------------------------------------------------------------------
